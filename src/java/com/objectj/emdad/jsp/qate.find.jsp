...
<body>
	<html:form method="post" action="/s/qate">
		<center>
			<span class="tilesHeader">انتخاب قطعه</span>
		</center>
		...
		<table width="100%" dir="rtl" align="center" class="generalText" border="0" cellpadding="3" cellspacing="0">
			<tr>
				<td class="filterFindEditCell" colspan="4">
			...
			<tr>
				...
				<td class="filterFieldCell"><html:text property="gheimat" size="7" maxlength="9" styleClass="field"/></td>
			</tr>
	
			<!------------------------------------------------>				
	
			<tr><td colspan="2" height="3">
				<input type="hidden" name="noeKhodroId" value="<%= request.getParameter( "noeKhodroId" ) %>" />
			</td></tr>
			...
			<logic:iterate id="value_object" name="value_object_list" >
				<tr class="listRow<%=i%2%>">
					...
					<td class = "listRow1Col">
						<a  href="javascript:selectQate( '<bean:write property="shomareFanni" name="value_object"/>', '<bean:write property="name" name="value_object"/>' );">
						<span id="shomareFanni<%=i%>"><bean:write property="shomareFanni" name="value_object"/></span>
						</a>
					</td>
...
<script>
	...
	function changePage(page) {
		...
		theForm.action="/emdad/s/qate.run";
		theForm.submit();
	}
	...
	function select(index) {
		creator = top.creator;
		if (creator!=null) {
			elem2 = document.getElementById("shomareFanni" + index);
			if (creator.document.tasviehForm!=null) {
				theId = creator.document.tasviehForm.cur_item.value;
				elem = creator.document.getElementById( theId );
				elem.value = elem2.innerText;
				elem.changed = true;
				
				elem = creator.document.getElementById( theId+"Onvan" );
				elem3 = document.getElementById("name" + index);
				elem.innerHTML = elem3.innerText;
			} else {
				creator.shomareFanni.value = elem2.innerText;
				creator.shomareFanni.changed = true;
				//creator.qateId.value = id;
				//creator.qateId.changed = true;
			}
		}
		creator.focus();
	}
	...
	function selectQate( id, onvan ) {
		...
		if (creator!=null) {
			...
			str = theId.split( "i" );
			creator.document.getElementById( "tedadQate" + str[1] ).value = "1";
			//creator.focus();
		}		
	}
